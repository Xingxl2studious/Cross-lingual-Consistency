    ...
    for domain in domains_list:
        ...
        for idx in range(len(data)):
            for lang in language_list:
                ...
                grdTruth = data[idx][lang]['groundTruth']
                # all_result.append(data[idx][lang]['answer'])
                data[idx][lang]['chrf_score'] = chrf.compute(predictions=[ans], references=[grdTruth], lowercase=True)['score']
                domain_chrf_collect[lang].append(data[idx][lang]['chrf_score'])
...
# nohup python xAC.py  > result_log/xAC.log 2>&1 &